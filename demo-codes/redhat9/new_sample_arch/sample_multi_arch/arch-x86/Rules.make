...
$(PRJ_NAME).elf: .depend $(DEP_FILES) $(START_OBJ) $(OBJS) 
	...
	$(CC) $(OBJS) -o $(PRJ_NAME).elf
	@echo $(OBJDUMP) -S $(PRJ_NAME).elf ">" $(PRJ_NAME).s
	@$(OBJDUMP) -S $(PRJ_NAME).elf > $(PRJ_NAME).s
	@echo $(READELF) -a $(PRJ_NAME).elf ">" $(PRJ_NAME).r
	@$(READELF) -a $(PRJ_NAME).elf > $(PRJ_NAME).r
	@echo $(NM)	$(PRJ_NAME).elf ">" $(PRJ_NAME).n
	@$(NM)	$(PRJ_NAME).elf > $(PRJ_NAME).n
	@echo
